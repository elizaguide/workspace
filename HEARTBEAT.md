# HEARTBEAT.md - Memory Maintenance & Proactive Tasks

## Memory Maintenance (Every Few Days)
- [ ] Review recent daily files (memory/YYYY-MM-DD.md) 
- [ ] Update MEMORY.md with significant insights/changes
- [ ] Check branch files for outdated information
- [ ] Update heartbeat-state.json timestamps

## Branch-Specific Maintenance
- [ ] **Instagram:** Check for new content ideas, account status changes
- [ ] **Twitter:** Monitor @elizaguide restrictions, update capabilities  
- [ ] **Spanish:** Review Vishen's learning progress, plan next lessons
- [ ] **AI Education:** Update demo capabilities, refine class materials
- [ ] **Partnership:** Capture relationship evolution, communication improvements
- [ ] **Personal Growth:** Note new development areas or breakthrough insights
- [ ] **Kids/London:** Populate with new context as it emerges

## Proactive Checks (Rotate Daily)
- **Monday:** Email scan for urgent items
- **Tuesday:** Calendar check for upcoming events (24-48h window)  
- **Wednesday:** Social media mentions and engagement
- **Thursday:** Memory maintenance day
- **Friday:** Project status review
- **Weekend:** Light monitoring only unless urgent

## Context Optimization
- Identify which branches are most active
- Merge overlapping information between branches
- Archive outdated branch content to prevent bloat
- Cross-reference related memories across branches