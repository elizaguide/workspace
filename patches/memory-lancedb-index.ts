...
const MEMORY_TRIGGERS = [
  ...
  /decided|agreed|plan is|strategy|goal|deadline/i,
  ...
  // Learning & progress triggers
  /learned|mastered|struggling|progress|lesson/i,
  // Feedback & correction triggers
  /don't|stop|change|fix|adjust|instead/i,
];
...
function shouldCapture(text: string): boolean {
  ...
  // Skip emoji-heavy responses (likely agent output, relaxed threshold)
    ...
    if (cfg.autoCapture) {
      api.on("agent_end", async (event) => {
        ...
        try {
          ...
          let stored = 0;
          for (const text of toCapture.slice(0, 8)) {
